...
class TranslationOrchestrator:
    ...
    def _register_prompts(self) -> None:
        ...
        backend_dir = Path(__file__).parent.parent.parent  # Go up from app/translation to backend
        project_root = backend_dir.parent  # Go up from backend to project root
        prompts_dir = project_root / "prompts"
        
        prompts_to_register = [
            ("code_translation", prompts_dir / "code_translation.txt", "Code translation prompt"),
        ]
        ...
        for prompt_name, prompt_file, description in prompts_to_register:
            try:
                ...
                logger.info(
                    f"Registered prompt '{prompt_name}' v1.0.0 from {prompt_file}",
                    extra={"stage_name": "translation_orchestration"}
                )
